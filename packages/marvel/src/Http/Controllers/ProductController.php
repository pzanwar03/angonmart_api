...
class ProductController extends CoreController
{
    ...
    public function latestProducts(Request $request)
    {
        ...
        return $this->fetchLatestProducts($request)->take($limit)->get();
    }
     ...
     * @return object
     */
    public function fetchLatestProducts(Request $request)
    {
        $type_slug = $request->type_slug;
        $language = $request->language ? $request->language : DEFAULT_LANGUAGE;
        $query = $this->repository->with(['type'])->where('language', $language);

        if ($type_slug) {
            $type = Type::where('slug', $type_slug)->where('language', $language)->first();
            $query = $this->repository->with(['type'])->where('type_id', $type->id)->where('language', $language)->latest();
            return $query;
        }
