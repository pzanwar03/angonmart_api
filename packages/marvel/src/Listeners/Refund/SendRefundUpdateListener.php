...
use Marvel\Notifications\RefundUpdateNotifications;
use Marvel\Traits\OrderSmsTrait;
use Marvel\Traits\SmsTrait;

class SendRefundUpdateListener implements ShouldQueue
{
    ...
    public function handle(RefundUpdate $event)
    {
        ...
        if ($emailReceiver['customer'] && $refund->customer()) {
            $refund->customer->notify(new RefundUpdateNotifications($refund, 'customer'));
        }

        if ($emailReceiver['admin']) {
            $admins = $this->adminList();
            foreach ($admins as $admin) {
                $admin->notify(new RefundUpdateNotifications($refund, 'admin'));
            }
        }
    }
}
